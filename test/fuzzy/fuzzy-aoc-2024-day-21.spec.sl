...
log("Part 1", codes.map(fn(code) -> Int(code[..-1]) * calc_presses(code, 2, false, nil)).sum()) // [expect] Part 1 215374
log("Part 2", codes.map(fn(code) -> Int(code[..-1]) * calc_presses(code, 25, false, nil)).sum()) // [expect] Part 2 260586897262600
